...
function savestudentdata() {
	...
	var url = "./resources/studentSurveyService/save";
	$.ajax({
		'type' : 'POST',
		'headers' : {
...
function fetchstudentsdata() {
	...
	var url = "./resources/studentSurveyService/get";
	$.ajax({
		'type' : 'GET',
		'headers' : {
			'Accept' : 'application/json',
		...
		'success' : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var cellText = rowObj.firstName ? document
						.createTextNode(rowObj.firstName) : document
						.createTextNode("N/A");
				cell.appendChild(cellText);
				row.appendChild(cell);

				var cell = document.createElement("td");
				var cellText = rowObj.streetAddress ? document
						.createTextNode(rowObj.streetAddress) : document
						.createTextNode("N/A");
				cell.appendChild(cellText);
				row.appendChild(cell);

				var cell = document.createElement("td");
				var cellText = rowObj.telphoneNumber ? document
						.createTextNode(rowObj.telphoneNumber) : document
						.createTextNode("N/A");
				cell.appendChild(cellText);
				row.appendChild(cell);

				var cell = document.createElement("td");
				var cellText = rowObj.email ? document
						.createTextNode(rowObj.email) : document
						.createTextNode("N/A");
				cell.appendChild(cellText);
				row.appendChild(cell);
